const { default: HoverDropdown } = require("@/app/components/dropDown/hoverDropDown/hoverDropDown");
const { default: LinearProgress } = require("@/app/components/progressbar/linearProgress");
const { default: Image } = require("next/image");
const { CgOptions } = require("react-icons/cg");

const ProjectsTable = ({ projects, formatDate, dropdownOptions, onOptionSelect }) => {
    return (
      <table className="border-x min-w-full table-auto">
        ...
        <tbody className="text-sm">
          {projects.map((project, i) => (
            <tr
              key={i}
              className="border-y hover:bg-gray-50 transition-colors duration-300 cursor-pointer"
            >
